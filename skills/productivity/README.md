...
`ponytail` used to live here. It is now installed as a Claude Code plugin
(user-level, `~/.claude/plugins`), so the repo no longer vendors a copy that
drifts from upstream. `sdd-apply` and `project-bootstrap` still invoke it.
